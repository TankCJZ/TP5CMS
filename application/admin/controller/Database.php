...
class Database extends Common
{
    ...
    {
        //列出备份文件列表
        $path = config('back.database_backup_path');
        if (!is_dir($path)) {
    ...
    public function downfile()
    {
        ...
        $name = $file . '-*.sql*';
        $filePath = glob(realpath(config('back.database_backup_path')) . DIRECTORY_SEPARATOR . $name);
    ...
    /**
     * 删除备份文件
     * @param  String $file 备份文件
     */
    public function del($file = '')
    {
        if ($file) {
            $name = $file . '-*.sql*';
            $path = realpath(config('back.database_backup_path')) . DIRECTORY_SEPARATOR . $name;
    ...
    public function import($time = 0, $part = null, $start = null)
    {
        if (is_numeric($time) && is_null($part) && is_null($start)) {
            ...
            $db = new \database\Database($list[$part], array('path' => realpath(config('back.database_backup_path')) . DIRECTORY_SEPARATOR, 'compress' => $list[$part][2]));
    ...
    public function export($tables = null, $id = null, $start = null)
    {
        if (request()->isPost() && !empty($tables) && is_array($tables)) {
            ...
            $config = array(
                ...
                'part' => config('back.database_backup_part_size'),
                'compress' => config('back.database_backup_compress'),
                'level' => config('back.database_backup_compress_level')
            );
            //检查是否有正在执行的任务
